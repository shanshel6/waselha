...
import { Plane, Package, MapPin, User, Weight, MessageSquare, Phone, CalendarDays, BadgeCheck, DollarSign, CheckCircle, XCircle, Clock, Trash2, Inbox, ArrowRight } from 'lucide-react';
...
export const ReceivedRequestsTab = ({ user, onUpdateRequest, updateRequestMutation, onCancelAcceptedRequest, onReviewChanges, reviewChangesMutation }: ReceivedRequestsTabProps) => {
  ...
  const { data: receivedRequests, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      if (!user) return [];
      const { data: requests, error: requestsError } = await supabase.from('requests').select(`*, trips(*)`).eq('trips.user_id', user.id).order('created_at', { ascending: false });
      if (requestsError) throw new Error(requestsError.message);
      const senderIds = [...new Set(requests.map(r => r.sender_id))];
      const { data: senderProfiles, error: profilesError } = await supabase.from('profiles').select('id, first_name, last_name, phone').in('id', senderIds);
      if (profilesError) throw new Error(profilesError.message);
      const profileMap = senderProfiles.reduce((acc, profile) => { acc[profile.id] = profile; return acc; }, {} as Record<string, Profile>);
      return requests.map(request => ({ ...request, sender_profile: profileMap[request.sender_id] || null, traveler_profile: null })) as RequestWithProfiles[];
    },
    enabled: !!user,
  });
  ...
  const calculatePriceDisplay = (request: RequestWithProfiles, useProposed = false) => {
    ...
    return { totalPriceUSD: result.totalPriceUSD, totalPriceIQD, pricePerKgUSD: result.pricePerKgUSD };
  };
    ...
    return (
      <div className="mt-4 p-4 border rounded-lg bg-green-100 dark:bg-green-900/30 space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
          <p className="flex items-center gap-2"><User className="h-4 w-4 text-primary" /><span className="font-semibold">{t('sender')}:</span>{otherPartyName}</p>
        ...
        <div className="border-t pt-3 space-y-2 text-sm">
          ...
          <p className="flex items-center gap-2"><MapPin className="h-4 w-4 text-primary" /><span className="font-semibold">{t('handoverLocation')}:</span>{req.handover_location || t('toBeDeterminedInChat')}</p>
        </div>
    ...
    return (
      <div className="mt-4 p-4 border-2 border-blue-500/50 rounded-lg bg-blue-50 dark:bg-blue-900/20 space-y-3">
        ...
        <div className="space-y-2 text-sm">
          <div className="flex items-center gap-2"><Weight className="h-4 w-4" /><span className="font-semibold">{t('packageWeightKg')}:</span><span className="line-through text-muted-foreground">{req.weight_kg} kg</span><ArrowRight className="h-4 w-4" /><span className="font-bold">{req.proposed_changes.weight_kg} kg</span></div>
          <div><p className="font-semibold flex items-center gap-2"><Package className="h-4 w-4" />{t('packageContents')}:</p><p className="line-through text-muted-foreground pl-6">{req.description}</p><p className="font-bold pl-6">{req.proposed_changes.description}</p></div>
        ...
        <div className="flex gap-2 pt-2">
          <Button size="sm" onClick={() => onReviewChanges({ request: req, accept: true })} disabled={reviewChangesMutation.isPending}>{t('acceptChanges')}</Button>
          <Button size="sm" variant="destructive" onClick={() => onReviewChanges({ request: req, accept: false })} disabled={reviewChangesMutation.isPending}>{t('rejectChanges')}</Button>
        </div>
      </div>
    );
  };
  ...
  if (error) return <div><p className="text-red-500">Error loading received requests: {error.message}</p><p className="text-sm text-muted-foreground mt-2">User ID: {user?.id}</p></div>;

  return (
    <Card>
      <CardHeader><CardTitle className="flex items-center gap-2"><Inbox className="h-6 w-6 text-primary" />{t('receivedRequests')}</CardTitle></CardHeader>
              ...
              <CardContent className="space-y-3">
                {hasPendingChanges ? renderProposedChanges(req) : (
                  <>
                    ...
                    {req.status === 'pending' && <div className="flex gap-2 pt-2"><Button size="sm" onClick={() => onUpdateRequest(req, 'accepted')} disabled={updateRequestMutation.isPending}>{t('accept')}</Button><Button size="sm" variant="destructive" onClick={() => onUpdateRequest(req, 'rejected')} disabled={updateRequestMutation.isPending}>{t('reject')}</Button></div>}
                  </>
                )}
              </CardContent>
            </Card>
          );
        }) : !isLoading && !error && <div><p>{t('noReceivedRequests')}</p>{user && <div className="mt-4 p-4 bg-yellow-50 dark:bg-yellow-900/30 rounded-lg"><p className="text-sm text-muted-foreground font-semibold">Debug Info:</p><p className="text-sm">User ID: {user.id}</p><p className="text-sm">Session: {user ? 'Active' : 'None'}</p></div>}</div>}
      </CardContent>
    </Card>
  );
};